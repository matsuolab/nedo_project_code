...
```sh
$ cd ~/ucllm_nedo_prod_kawagoshi/pipeline/0_download_dirty_datasets
$ pwd
// ~/ucllm_nedo_prod_kawagoshi/pipeline/0_download_dirty_datasets
...
部分的なダウンロードを行う場合
```sh
$ python -m download_dirty_datasets.download --dataset=c4 --split=train --output_base="~/ucllm_nedo_prod_kawagoshi/pipeline/0_download_dirty_datasets/output" --index_from=0 --index_to=10
```

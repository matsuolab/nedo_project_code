...
# 4/24 Set PYTORCH_CUDA_ALLOC_CONF to avoid memory fragmentation
...
wandb_project="pretrain_mistral_node1"
...
# model_size = 12 * n_layers * hidden_size ** 2
# 5B: n_layers = 24, hidden_size = 4096, ffn_hidden = 4096*4

## Mistral 5B
#seq_len=2048 # 85% of dataset has seq_len < 4096. To save memory, change from 8196
#model="mistral_5b"
#model_size=7
#num_layers=24 #7b  32
#hidden_size=4096
#ffn_hidden_size=16384  #14336
#num_attn_heads=16
#num_key_value_heads=4
#window_size=2048
#window_size=1024
...
NHOSTS=1
echo "NHOST value: ${NHOSTS}"
if [ -z "${NHOSTS}"]; then
	echo "Error: NHOSTS is not set"
	exit 1
else
	num_node="${NHOSTS}"
fi
...
data_path="/storage3/tokenized_data/merged_japanese_100B_text_document"
...
# to enable zero_stage=2, delete --pipeline-model-parallel-size ${pp_size}

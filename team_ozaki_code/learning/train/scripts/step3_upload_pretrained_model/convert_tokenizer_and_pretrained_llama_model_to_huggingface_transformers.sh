...
while [[ ${#} -gt 0 ]]; do
    case ${1} in
        ...
        --output_megatron_dir) output_megatron_dir=${2}; shift ;;
...
if [[ -z ${input_tokenizer_file} ]] || [[ -z ${input_model_dir} ]] || [[ -z ${output_megatron_dir} ]]|| [[ -z ${output_tokenizer_and_model_dir} ]]; then
    echo "Error: Missing required arguments."
    echo "Usage: ${0} --input_tokenizer_file <input_tokenizer_file> --input_model_dir <input_model_dir> --output_megatron_dir <output_megatron_dir> --output_tokenizer_and_model_dir <output_tokenizer_and_model_dir>"
    exit 1
fi
...
mkdir -p ${output_megatron_dir}

# Converts the pretrained model from Megatron-DeepSpeed format to Megatron format.
python ${megatron_deepspeed_dir}/tools/convert_checkpoint/deepspeed_to_megatron.py \
    --input_folder ${input_model_dir} \
    --output_folder ${output_megatron_dir} \
    --target_pp ${target_pp} \
    --target_tp ${target_tp} \

# Converts the pretrained model from Megatron format to HuggingFace Transformers format.
python ${ucllm_nedo_dev_train_dir}/scripts/step3_upload_pretrained_model/llama_checkpoint_conversion.py \
    --convert_checkpoint_from_megatron_to_transformers \
    --load_path ${output_megatron_dir} \

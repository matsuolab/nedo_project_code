...
# created     : 10/4/22
# 2024/4/13 delete_files 関数を追加 by someya
...
def delete_files(io_args: IOArgs):
    """
    Deletes files matching 'data-*.arrow' in the specified data directory.
    """
    if io_args.path and os.path.isdir(io_args.path):
        pattern = re.compile(r"data-.*\.arrow$")
        for filename in os.listdir(io_args.path):
            if pattern.match(filename):
                file_path = os.path.join(io_args.path, filename)
                try:
                    os.remove(file_path)
        ...
        print(f"Invalid or unspecified data directory: {io_args.path}")

...
import wandb
from accessory.util.wandb_utils import log_model_info
...
def get_args_parser():
    ...
    parser.add_argument('--data_parallel', type=str, choices=['sdp', 'fsdp',"hsdp","hsdp2"], default='sdp')
...
def main(args):
    ...
    # ログ設定
    logging.basicConfig(filename=f'node_{node_id}.log', level=logging.DEBUG)

    # ログメッセージの出力
    logging.info("This is an info message")
    # 環境変数の出力
    logging.debug('MASTER_PORT: %s', os.environ.get('MASTER_PORT'))
    logging.debug('MASTER_ADDR: %s', os.environ.get('MASTER_ADDR'))
    logging.debug('SLURM_NPROCS: %s', os.environ.get('SLURM_NPROCS'))
    logging.debug('SLURM_PROCID: %s', os.environ.get('SLURM_PROCID'))
    logging.debug('LOCAL_RANK: %s', os.environ.get('LOCAL_RANK'))
    logging.debug('WORLD_SIZE: %s', os.environ.get('WORLD_SIZE'))
    logging.debug('RANK: %s', os.environ.get('RANK'))
    ...
    logging.debug('MP_rank: %s', mp_rank)
    logging.debug('DP_rank: %s', dp_rank)
    logging.debug('DP_ws: %s', dp_world_size)
    logging.debug('DP_g: %s', dp_group)
   ...
   # wandb setting
    ...
    model = FSDP(
        ...
        sharding_strategy={
            ...
            "hsdp2": ShardingStrategy._HYBRID_SHARD_ZERO2,
    ...
    if int(os.environ.get('LOCAL_RANK')) == 0 and args.output_dir is not None:

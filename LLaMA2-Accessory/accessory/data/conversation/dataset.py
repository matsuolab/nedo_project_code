...
class FinetuneDialogDataset(Dataset):
    ...
    def get_item_func(self, index):
        ...
        if len(to_predict_values) == 0:
            ...
            return self[index-1]
        #print("会話:",conversation)
        #print("予測値:",to_predict_values)
        ...
        for value in to_predict_values:
            tokenized_value = self.tokenizer.encode(value, bos=False, eos=True)
            tokenized_value_no_prefix_space = tokenized_value[1:] # 先頭の空白は除外
            tokenized_value_for_find = tokenized_value[1:-1] # 先頭の空白とeosを除外
            #例外処理

            value_pos = find_sublist(tokenized_conversation[check_pos:], tokenized_value_for_find) + check_pos
            #print("tokenizer会話",tokenized_conversation)

            #print("tokenizer予測値",tokenized_value)
            if value_pos == -1:
                print("a sentence mismatches the corresponding piece in the conversation")
                return self[index-1]
            labels[value_pos:value_pos+len(tokenized_value[1:])] = tokenized_value[1:]

            #print("labels:",str(multi_count)+"回",labels)
            multi_count +=1
            assert labels[value_pos:value_pos+len(tokenized_value[1:-1])] == tokenized_conversation[value_pos:value_pos+len(tokenized_value[1:-1])]
            #if labels[value_pos:value_pos+len(tokenized_value[1:-1])] != tokenized_conversation[value_pos:value_pos+len(tokenized_value[1:-1])]:
               #print("ValuePos:",value_pos)
               #print("tokenizer会話",tokenized_conversation)
               #print("tokenizer予測値",tokenized_value)
               #print("回答:",labels[value_pos:value_pos+len(tokenized_value[1:-1])])
               #print(value)
               #print("問題:",tokenized_conversation[value_pos:value_pos+len(tokenized_value[1:-1])])
               #print(conversation)
            #assert labels[value_pos:value_pos+len(tokenized_value)] == tokenized_conversation[value_pos:value_pos+len(tokenized_value)]
            check_pos = value_pos+len(tokenized_value)-2

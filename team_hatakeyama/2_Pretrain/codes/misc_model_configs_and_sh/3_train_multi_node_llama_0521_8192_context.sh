...
megatron_deepspeed_dir=$(yq -r '.megatron_deepspeed_dir' multinode_config_llama_8192_context.yaml)
input_tokenizer_file=$(yq -r '.input_tokenizer_file' multinode_config_llama_8192_context.yaml)
tokenized_data_path=$(yq -r '.tokenized_data_path' multinode_config_llama_8192_context.yaml)
output_model_dir=$(yq -r '.output_model_dir' multinode_config_llama_8192_context.yaml)
save_interval=$(yq -e '.save_interval' multinode_config_llama_8192_context.yaml)
...
model_size=$(yq -e '.model_size' multinode_config_llama_8192_context.yaml)
num_layers=$(yq -e '.num_layers' multinode_config_llama_8192_context.yaml)
hidden_size=$(yq -e '.hidden_size' multinode_config_llama_8192_context.yaml)
num_attn_heads=$(yq -e '.num_attn_heads' multinode_config_llama_8192_context.yaml)
global_batch_size=$(yq -e '.global_batch_size' multinode_config_llama_8192_context.yaml)
lr=$(yq -e '.lr' multinode_config_llama_8192_context.yaml)
min_lr=$(yq -e '.min_lr' multinode_config_llama_8192_context.yaml)
init_std=$(yq -e '.init_std' multinode_config_llama_8192_context.yaml)
seq_len=$(yq -e '.seq_len' multinode_config_llama_8192_context.yaml)
...
train_tokens=$(yq -e '.train_tokens' multinode_config_llama_8192_context.yaml)

#ここを適当に大きくしすぎると､必要メモリが増えすぎるので注意｡
##30000*...とかにすると､RAMが600GB必要､みたいになる
#train_samples=$(( 300 * 1000 * 1000 * 1000 * 2 / ${seq_len} ))

train_samples=$(yq -e '.train_samples' multinode_config_llama_8192_context.yaml)
...
lr_decay_tokens_in_billion=$(yq -e '.lr_decay_tokens_in_billion' multinode_config_llama_8192_context.yaml)
lr_decay_tokens=$(yq -e '.lr_decay_tokens' multinode_config_llama_8192_context.yaml)
...
mp_size=$(yq -e '.mp_size' multinode_config_llama_8192_context.yaml)

## Pipeline parallelism. To disable PP, set pp_size to 1 and no_pp to true.
## Note that currently both curriculum learning and random-LTD are NOT
## compatible with pipeline parallelism.
pp_size=$(yq -e '.pp_size' multinode_config_llama_8192_context.yaml)
...
zero_stage=$(yq -e '.zero_stage' multinode_config_llama_8192_context.yaml)
...
NHOSTS=$(yq -e '.NHOSTS' multinode_config_llama_8192_context.yaml)
num_node="${NHOSTS}"
num_gpus=$((${num_gpus_pernode} * ${num_node}))
## Data parallel size.
sp_size=$(yq -e '.sp_size' multinode_config_llama_8192_context.yaml)
## Data parallel size.
...
micro_batch_size=$(yq -e '.micro_batch_size' multinode_config_llama_8192_context.yaml)
# batch_size=2
###############################################################################
### Misc configs
log_interval=1
...
activation_checkpoint=$(yq -r '.activation_checkpoint' multinode_config_llama_8192_context.yaml)
...
num_workers=$(yq -e '.num_workers' multinode_config_llama_8192_context.yaml)
...
megatron_options=" \
    ...
    --bf16 \
    --seed ${seed} \
    --load /storage5/checkpoints/0514llama_epoch2//checkpoint/gpt_8B_tok300B_lr5.8e-05_min2e-06_w400M_d64B_cosine_gbs1536_mbs3_g24_z1_pp3_seed1234_rebase \

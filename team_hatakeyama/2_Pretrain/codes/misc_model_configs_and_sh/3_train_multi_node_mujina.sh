...
megatron_deepspeed_dir=$(yq -r '.megatron_deepspeed_dir' /storage5/llm/codes/2_pretrain/multinode_config_mujina.yaml)
input_tokenizer_file=$(yq -r '.input_tokenizer_file' /storage5/llm/codes/2_pretrain/multinode_config_mujina.yaml)
tokenized_data_path=$(yq -r '.tokenized_data_path' /storage5/llm/codes/2_pretrain/multinode_config_mujina.yaml)
output_model_dir=$(yq -r '.output_model_dir' /storage5/llm/codes/2_pretrain/multinode_config_mujina.yaml)
save_interval=$(yq -e '.save_interval' /storage5/llm/codes/2_pretrain/multinode_config_mujina.yaml)
...
model_size=$(yq -e '.model_size' /storage5/llm/codes/2_pretrain/multinode_config_mujina.yaml)
num_layers=$(yq -e '.num_layers' /storage5/llm/codes/2_pretrain/multinode_config_mujina.yaml)
hidden_size=$(yq -e '.hidden_size' /storage5/llm/codes/2_pretrain/multinode_config_mujina.yaml)
num_attn_heads=$(yq -e '.num_attn_heads' /storage5/llm/codes/2_pretrain/multinode_config_mujina.yaml)
global_batch_size=$(yq -e '.global_batch_size' /storage5/llm/codes/2_pretrain/multinode_config_mujina.yaml)
lr=$(yq -e '.lr' /storage5/llm/codes/2_pretrain/multinode_config_mujina.yaml)
min_lr=$(yq -e '.min_lr' /storage5/llm/codes/2_pretrain/multinode_config_mujina.yaml)
init_std=$(yq -e '.init_std' /storage5/llm/codes/2_pretrain/multinode_config_mujina.yaml)
seq_len=$(yq -e '.seq_len' /storage5/llm/codes/2_pretrain/multinode_config_mujina.yaml)
...
train_tokens=$(yq -e '.train_tokens' /storage5/llm/codes/2_pretrain/multinode_config_mujina.yaml)

#ここを適当に大きくしすぎると､必要メモリが増えすぎるので注意｡
##30000*...とかにすると､RAMが600GB必要､みたいになる
#train_samples=$(( 300 * 1000 * 1000 * 1000 * 2 / ${seq_len} ))

train_samples=$(yq -e '.train_samples' /storage5/llm/codes/2_pretrain/multinode_config_mujina.yaml)
...
## stability.
lr_warmup_tokens_in_million=150
...
lr_decay_tokens_in_billion=$(yq -e '.lr_decay_tokens_in_billion' /storage5/llm/codes/2_pretrain/multinode_config_mujina.yaml)
lr_decay_tokens=$(yq -e '.lr_decay_tokens' /storage5/llm/codes/2_pretrain/multinode_config_mujina.yaml)
...
mp_size=$(yq -e '.mp_size' /storage5/llm/codes/2_pretrain/multinode_config_mujina.yaml)

## Pipeline parallelism. To disable PP, set pp_size to 1 and no_pp to true.
## Note that currently both curriculum learning and random-LTD are NOT
## compatible with pipeline parallelism.
pp_size=$(yq -e '.pp_size' /storage5/llm/codes/2_pretrain/multinode_config_mujina.yaml)
...
zero_stage=$(yq -e '.zero_stage' /storage5/llm/codes/2_pretrain/multinode_config_mujina.yaml)
...
NHOSTS=$(yq -e '.NHOSTS' /storage5/llm/codes/2_pretrain/multinode_config_mujina.yaml)
num_node="${NHOSTS}"
num_gpus=$((${num_gpus_pernode} * ${num_node}))
## Data parallel size.
sp_size=$(yq -e '.sp_size' /storage5/llm/codes/2_pretrain/multinode_config_mujina.yaml)
## Data parallel size.
...
micro_batch_size=$(yq -e '.micro_batch_size' /storage5/llm/codes/2_pretrain/multinode_config_mujina.yaml)
# batch_size=2
###############################################################################
### Misc configs
log_interval=1
...
activation_checkpoint=$(yq -r '.activation_checkpoint' /storage5/llm/codes/2_pretrain/multinode_config_mujina.yaml)
...
num_workers=$(yq -e '.num_workers' /storage5/llm/codes/2_pretrain/multinode_config_mujina.yaml)
...
megatron_options=" \
    ...
    --bf16 \
    --seed 3 \
    --load /storage5/llm/codes/2_pretrain/mujina_test_pth/ \

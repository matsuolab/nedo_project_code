...
def main(base_dir, index_from, index_to, num_worker=8):
    for index in range(index_from, index_to+1):
        path_input = Path(base_dir).joinpath(f'c4-ja.tfrecord-{index:05}-of-01024.json').as_posix()
        path_output = f'/persistentshare/storage/team_haijima/dataset_pre/20240418_preprocess/datasets/mC4/c4-ja-{index:05}-of-01024.jsonl'
        process_json(path_input, path_output, 
                     [('map', map_function_mC4), ('filter', filter_function), ('map', map_function_dedup)],
...
if __name__ == '__main__':
    ...
    base_dir = '/persistentshare/storage/team_haijima/yamauchi/tmp/allenai/c4/multilingual/'

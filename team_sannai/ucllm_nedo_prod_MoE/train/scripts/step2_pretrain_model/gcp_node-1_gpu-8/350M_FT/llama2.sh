...
while [[ ${#} -gt 0 ]]; do
    case ${1} in
        ...
        --dataset_index) dataset_index=${2}; shift ;;
        --dataset_type) dataset_type=${2}; shift ;;
        --train_tokens_rate) train_tokens_rate=${2}; shift ;;
        --checkpoint_path) checkpoint_path=${2}; shift ;;
        --train_tokens_in_million) train_tokens_in_million=${2}; shift ;;
        --freeze_attn) freeze_attn=${2}; shift ;;
        --freeze_head) freeze_head=${2}; shift ;;
        ## option args
        --save_interval) save_interval=${2}; shift ;;
        --lr_warmup_steps) lr_warmup_steps=${2}; shift ;;
...
if [[ -z ${input_tokenizer_file} ]] || [[ -z ${output_model_dir} ]] || [[ -z ${checkpoint_path} ]] || [[ -z ${dataset_index} ]] || [[ -z ${dataset_type} ]]; then
    echo "Error: Missing required arguments."
    echo "Usage: ${0} --input_tokenizer_file <input_tokenizer_file> --output_model_dir <output_model_dir> --checkpoint_path <checkpoint_path> --dataset_index <dataset_index> --dataset_type <dataset_type>"
    exit 1
fi
if [[ $dataset_type != "stack" ]] && [[ $dataset_type != "s2" ]] && [[ $dataset_type != "wiki" ]]; then
    echo "dataset_type is stack or s2 or wiki."
    exit 1
fi
...
echo "load path = ${checkpoint_path}"
echo "output_model_dir = ${output_model_dir}"
echo ""
...
rated=`echo "${train_tokens_in_million}*${train_tokens_rate}*1000*1000" | bc`
train_tokens=${rated%.*}
echo $num
...
train_samples=2799357 ## seed用のデータセット1, 2の行数
if [[ $dataset_type == "stack" ]]; then
    train_samples=632667
fi
if [[ $dataset_type == "s2" ]]; then
    train_samples=16458
fi
if [[ $dataset_type == "wiki" ]]; then
    train_samples=831329
fi
...
# fi
# echo ""
ds_base_path="/persistentshare/storage/team_sannai/team_la/ft"
if [[ $dataset_type == "stack" ]]; then
    data_path="\
        1 ${ds_base_path}/dataset${dataset_index}/stack/stack_0_text_document \
        1 ${ds_base_path}/dataset${dataset_index}/stack/stack_1_text_document"
fi
if [[ $dataset_type == "s2" ]]; then
    data_path="${ds_base_path}/dataset${dataset_index}/s2/s2_text_document"
fi
if [[ $dataset_type == "wiki" ]]; then
    data_path="${ds_base_path}/dataset${dataset_index}/wiki/wiki_text_document"
fi

prescale_grad="true"
jobname="gpt_${model_size}B_tok${train_tokens_in_million}B"
jobname="${jobname}_lr${lr}_min${min_lr}_w${lr_warmup_tokens}M_d${lr_decay_tokens}B_${lr_decay_style}"
...
output_model_path="${output_model_dir}/checkpoint/${jobname}"
...
## --- finetune options ---
if [ "${freeze_attn}" = "true" ]; then
megatron_options="${megatron_options} \
    --freeze_attn"
fi
if [ "${freeze_head}" = "true" ]; then
megatron_options="${megatron_options} \
    --freeze_head"
fi
## --- finetune options ---
...
iteration_file="${output_model_path}/latest_checkpointed_iteration.txt"
iteration_file_2="${output_model_path}/latest"
iteration=0

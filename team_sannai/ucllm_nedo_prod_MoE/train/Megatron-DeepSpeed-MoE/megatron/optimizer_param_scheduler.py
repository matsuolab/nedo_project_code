...
class WarmupStableDecayScheduler(OptimizerParamScheduler):
    def __init__(self, stable_train_steps, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.stable_train_steps = stable_train_steps
        args = get_args()
        # self.alpha = args.wsd_alpha
        self.tmp = args.wsd_tmp
    def get_lr(self):
        if self.num_steps == 0:
            return 1/self.lr_warmup_steps * self.max_lr
        elif self.num_steps <= self.lr_warmup_steps:
            return self.num_steps/self.lr_warmup_steps * self.max_lr
        elif self.num_steps <= self.stable_train_steps:
            return self.max_lr
        elif self.stable_train_steps < self.num_steps:
            # f = math.exp(-self.alpha*(self.num_steps - self.stable_train_steps))
            f = math.pow(0.5, (self.num_steps - self.stable_train_steps)/self.tmp)
            return f * self.max_lr
        else:
            ## ここには入らないはず
            raise Exception('Error: WarmupStableDecayScheduler.get_lr...steps', self.num_steps)

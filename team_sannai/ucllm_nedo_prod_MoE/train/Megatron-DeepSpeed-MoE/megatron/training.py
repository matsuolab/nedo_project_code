...
from megatron.optimizer_param_scheduler import OptimizerParamScheduler, WarmupStableDecayScheduler
...
def get_model(model_provider_func, model_type=ModelType.encoder_or_decoder, wrap_with_ddp=True):
    ...
    if mpu.get_data_parallel_rank() == 0:
        params = sum([sum([p.ds_numel if hasattr(p,'ds_id') else p.nelement() for p in model_module.parameters()])
                 for model_module in model])
...
def get_optimizer_param_scheduler(optimizer):
    ...
    if args.use_wsd:
        stable_train_steps = args.stable_train_iters * args.global_batch_size
        opt_param_scheduler = WarmupStableDecayScheduler(
            stable_train_steps,
            optimizer,
            max_lr=args.lr,
            min_lr=args.min_lr,
...
def training_log(loss_dict, total_loss_dict, learning_rate, iteration,
                 ...
                 grad_norm, params_norm, num_zeros_in_grad, total_hour,
    ...
    if iteration % args.log_interval == 0:
        ...
        total_hour += elapsed_time / (3600) # second ->hour
        ...
        log_string += ' total hour:{:.2f} |'.format(total_hour)
        total_loss_dict[advanced_iters_key] = 0
    ...
    return report_memory_flag, total_hour
...
def train(forward_step_func, model, optimizer, opt_param_scheduler,
    ...
    while iteration < args.train_iters and (args.train_tokens is None or \
        ...
        report_memory_flag, total_hour = training_log(loss_dict, total_loss_dict,
...
def evaluate_and_print_results(prefix, forward_step_func,
    ...
    for key in total_loss_dict:
        ...
        string += '{} value: {:.6E} | '.format(key, loss_val)
        ...
        if writer and is_last_rank():
            ...
            if wandb is not None and getattr(wandb, 'run', None) is not None:
                wandb.log({
                    f'{data_type}/{key} loss': loss_val,
                    f'{data_type}/{key} PPL': ppl,

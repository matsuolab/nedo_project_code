...
def freeze_lmhead_layer(model):
    for _, module in model.named_modules():
        if isinstance(module, LMHeadPipe) or isinstance(module, EmbeddingPipe):
            for param in module.parameters():
                param.requires_grad = False
    return model
...
def add_user_define_args(parser):
    """Provide extra arguments required for tasks."""
    group = parser.add_argument_group(title='user_define')
    group.add_argument('--use-wsd',
                       action='store_true', default=False)
    group.add_argument('--stable-train-iters',type=int)
    # group.add_argument('--wsd-alpha',type=float)
    group.add_argument('--wsd-tmp',type=float)
    return parser
...
if __name__ == "__main__":
    ...
    pretrain(train_valid_test_datasets_provider,
             ...
             extra_args_provider=add_user_define_args
             )
